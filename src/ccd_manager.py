...
class CCD_Manager:
    def __init__(self, process: Optional[Callable[[NDArray], tuple[NDArray, NDArray]]] = None, fps: float = 10):
        self.__camera = CCD()
        self.__process = process or Image_Processor()
        self.__interval  = 1.0 / fps
        self.__last_time = 0.0

    def __publish(self, bayer: NDArray, on_publish: Callable[[NDArray, NDArray], None]) -> None:
        now = time.monotonic()
        if now - self.__last_time >= self.__interval:
            self.__last_time = now
            image, array = self.__process(bayer)
            on_publish(image, array)

    def start(self, on_publish: Callable[[NDArray, NDArray], None]) -> None:
